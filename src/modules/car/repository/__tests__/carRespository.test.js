...
describe('Testing car repository', () => {
  ...
  test('save creates a car the DB', async () => {
    ...
    const expectedCar = fromDataToEntity(car);
    expectedCar.id = 1;
  ...
  test('updates a car in the DB', async () => {
    const newCar = createCar(undefined);
    const carToUpdate = createCar(1);
    carToUpdate.brand = 'Test';
    await carRepository.save(newCar);
    const updatedCar = await carRepository.save(carToUpdate);
