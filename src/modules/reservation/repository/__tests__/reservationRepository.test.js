...
describe('Testing reservation repository', () => {
  ...
  test("save creates a reservation the DB", async ()=>{
    let reservation = createReservation(undefined);
    let savedReservation = await reservationRepository.save(reservation);
    let expectedReservation = fromDataToEntity(reservation);
  ...
  test("updates a reservation in the DB", async()=>{
    let newReservation = createReservation(undefined);
    let reservationToUpdate = createReservation(1);
    reservationToUpdate.paymentMethod = "Test"
    await reservationRepository.save(newReservation);
    let updatedCar = await reservationRepository.save(reservationToUpdate);
